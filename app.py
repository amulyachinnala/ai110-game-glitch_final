...
attempt_limit_map = {
    "Easy": 10,  ## easy should have more attempts than normal
    "Normal": 7,    ## normal should have less attempts than easy
    "Hard": 5,
}
...
if "attempts" not in st.session_state:
    st.session_state.attempts = 0   ## attempts start at 0 because the user hasn't made a guess yet
...
if new_game:
    ...
    st.session_state.secret = random.randint(low, high) ## limits assigned by difficulty
    st.session_state.status = "playing" ##add status
    st.session_state.score = 0 ##add score
    st.session_state.history = [] ##add history
...
##display the updated info last
if st.session_state.status == "playing":
    st.info(
        f"Guess a number between {low} and {high}. " ## limits based on difficulty
...
with st.expander("Developer Debug Info"):
    ...
    st.write("Staus:", st.session_state.status) ##include status
